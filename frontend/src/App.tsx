...
import { getSale, getPrenotazioni, creaPrenotazione, modificaPrenotazione, cancellaPrenotazione } from './api'
...
type PreCompila = { sala_id: number; data: string; inizio: string; fine: string }
...
function App() {
  ...
  const [prenotazioneInModifica, setPrenotazioneInModifica] = useState<Prenotazione | undefined>()
  const [preCompilaForm, setPreCompilaForm] = useState<PreCompila | undefined>()
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.from(containerRef.current, { opacity: 0, y: 30, duration: 0.6, ease: 'power2.out' })
    })
    return () => ctx.revert()
  }, [])
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        const [saleFetched, prenotazioniFetched] = await Promise.all([getSale(), getPrenotazioni()])
        if (!cancelled) { setSale(saleFetched); setPrenotazioni(prenotazioniFetched) }
      } catch (err: unknown) {
        if (!cancelled) setErroreCaricamento(err instanceof Error ? err.message : 'Errore di caricamento')
      } finally {
  ...
  async function caricaDati() {
    ...
    try {
      ...
      setErroreCaricamento(err instanceof Error ? err.message : 'Errore di caricamento')
    } finally {
  ...
  // Called when user clicks a free room in the availability checker
  ...
  function togglePannello(pannello: 'form' | 'disponibilita') {
    setPannelloAperto(p => {
      if (p === pannello) { setPrenotazioneInModifica(undefined); return null }
  ...
  function handleToggleTema() {
    gsap.to(toggleRef.current, {
      rotation: 360, duration: 0.35, ease: 'power2.out',
  ...
  return (
    <div className="container" ref={containerRef}>
      <div className="header">
        ...
        <button className="tema-toggle" ref={toggleRef} onClick={handleToggleTema}>
      ...
      {caricamento ? (
        ...
        <>
          {/* Toggle buttons — table stays visible below without scrolling */}
          <div className="pannelli-toggle">
            <button
              className={`btn-pannello${pannelloAperto === 'form' ? ' attivo' : ''}`}
              onClick={() => togglePannello('form')}
            >
              📝 {prenotazioneInModifica ? 'Modifica prenotazione' : 'Nuova prenotazione'}
            </button>
            <button
          ...
          <div className={`pannello${pannelloAperto === 'form' ? ' aperto' : ''}`}>
            <FormPrenotazione
              sale={sale}
              ...
              onAnnullaModifica={() => { setPrenotazioneInModifica(undefined); setPannelloAperto(null) }}
              preCompila={preCompilaForm}
            />
          </div>

          <div className={`pannello${pannelloAperto === 'disponibilita' ? ' aperto' : ''}`}>
            <DisponibilitaSale
              sale={sale}
              prenotazioni={prenotazioni}
